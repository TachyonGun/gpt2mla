...
REP_SEQUENCE = 50  # Set to False to use SENTENCE, or a number N to generate repeated sequence of length 2N
PLOT_SUBSET = 10   # Set to False to plot all points, or a number N to plot N points (or N points from each half if REP_SEQUENCE is set)
...
def load_mla_model(checkpoint_path):
    """
...
def project_to_2d(activations):
    """
    Project higher-dimensional activations to 2D using PCA
    """
    # Reshape to 2D array for PCA
    B, T, D = activations.shape
    flat_activations = activations.reshape(-1, D)
    
    # Fit and transform with PCA
    pca = PCA(n_components=2)
    projected = pca.fit_transform(flat_activations)
    
    # Reshape back to original batch and sequence dimensions
    return projected.reshape(B, T, 2), pca.explained_variance_ratio_

def extract_latent_activations(model, tokens):
    """
    Extract latent space activations and model predictions
    If dimensionality > 2, project to 2D using PCA
    """
    B, T = tokens.size()
    
    # Store latent activations from each layer before PCA
    raw_activations = []
    
    def latent_hook(module, input, output):
        # Extract the input and compute latent encoding
        x = input[0]
        z = module.latent_enc(x)  # Shape: [B, T, n_latent]
        raw_activations.append(z.detach().cpu())
    
    # Register hooks for all layers
    hooks = []
    for block in model.transformer.h:
        hooks.append(block.attn.register_forward_hook(latent_hook))
    ...
    # Process activations
    if raw_activations[0].shape[-1] > 2:
        # Combine activations from all layers for PCA
        all_activations = torch.stack(raw_activations, dim=0)  # [n_layers, B, T, n_latent]
        ...
        latent_activations = []
        for layer_acts in raw_activations:
            layer_flat = layer_acts.reshape(-1, D).numpy()
            layer_projected = pca.transform(layer_flat)
            latent_activations.append(torch.from_numpy(layer_projected.reshape(B, T, 2)))
        
        variance_explained = [pca.explained_variance_ratio_] * n_layers
    else:
        # If already 2D, just take first 2 dimensions
        latent_activations = [z[:, :, :2] for z in raw_activations]
        variance_explained = [None] * len(raw_activations)
    
    return latent_activations, logits, variance_explained

def plot_latent_activations(latent_activations, tokens, tokenizer, text, variance_explained):
    """
    Plot the 2D latent activations for each layer
    """
    n_layers = len(latent_activations)
    ...
    for layer_idx in range(n_layers):
        ...
        # Get latent activations for this layer
        z = latent_activations[layer_idx][0]  # [T, 2]
        ...
        # Add layer info as subtitle with variance explained if PCA was used
        if variance_explained[layer_idx] is not None:
            var_text = f"\nPCA var: {variance_explained[layer_idx][0]:.2%}, {variance_explained[layer_idx][1]:.2%}"
            title = f"Layer {layer_idx}{var_text}"
        else:
    ...
    plt.savefig("latent_activations.png", dpi=300, bbox_inches='tight')
    plt.savefig("latent_activations.svg", format='svg', bbox_inches='tight')
    plt.show()
...
def main():
    parser = argparse.ArgumentParser(description='Visualize latent space activations in the MLA model')
    parser.add_argument('--sentence', type=str, default=SENTENCE,
                        help='Input sentence to analyze')
    parser.add_argument('--checkpoint', type=str, default='out_mla/ckpt_mla.pt',
                        help='Path to the MLA model checkpoint')
    args = parser.parse_args()
    
    # Load model and tokenizer
    model = load_mla_model(args.checkpoint)
    ...
    # Extract latent activations and get predictions
    latent_activations, logits, variance_explained = extract_latent_activations(model, input_ids)
    
    # Plot activations
    plot_latent_activations(latent_activations, input_ids, tokenizer, args.sentence, variance_explained)
    
    # Analyze and display predictions
    analyze_predictions(model, input_ids, tokenizer)
    
    print(f"\nVisualizations saved as 'latent_activations.png' and 'latent_activations.svg'")
