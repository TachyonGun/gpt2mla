"""
Test script to compare standard GPT and Multi-Latent Attention (MLA) GPT 
on the Shakespeare character dataset.
"""
...
def prepare_data():
    """Load the Shakespeare character dataset"""
    ...
    return train_data, val_data, 65  # 65 is the vocab size for Shakespeare char
...
def train_model(model, train_data, val_data, model_type="GPT"):
...
def sample_text(model, model_type="GPT"):
    ...
    vocab_size = 65
    itos = {i: chr(i+ord('a')) if i < 26 else 
           chr(i-26+ord('A')) if i < 52 else
           '!' if i == 52 else
           ',' if i == 53 else
           '.' if i == 54 else
           ':' if i == 55 else
    ...
    generated_text = ''.join([itos[i] for i in generated_text])
...
def main():
    ...
    # Train both models
    gpt_losses = train_model(gpt_model, train_data, val_data, "GPT")
    mla_losses = train_model(mla_model, train_data, val_data, "MLA")
    
    # Generate sample text
    sample_text(gpt_model, "GPT")
    sample_text(mla_model, "MLA")
