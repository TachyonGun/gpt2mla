...
def compute_prev_token_scores(attention_patterns):
    """
    Compute scores for how much each head attends to the previous token.
    This is measured by looking at the sub-diagonal of the attention pattern.
    """
    ...
    for layer in range(n_layers):
        ...
        for head in range(n_heads):
            # Look at attention to previous token (sub-diagonal elements)
            prev_token_attention = pattern[head].diagonal(-1)  # Offset by -1 to get sub-diagonal
            scores[layer, head] = prev_token_attention.mean()
    
    return scores

def plot_prev_token_scores(scores, model_name):
    """
    Create a heatmap of previous token attention scores using plotly
    """
    fig = px.imshow(
        scores.cpu().numpy(),
        labels=dict(x="Head", y="Layer", color="Previous Token Score"),
        title=f"Previous Token Attention Scores for {model_name}",
...
def main():
    ...
    gpt2_scores = compute_prev_token_scores(gpt2_patterns)
    plot_prev_token_scores(gpt2_scores, "GPT-2")
    ...
    mla_scores = compute_prev_token_scores(mla_patterns)
    plot_prev_token_scores(mla_scores, "MLA")

    # Print heads with high previous token scores (threshold = 0.4)
    threshold = 0.4
    print("\nGPT-2 Previous Token Heads:")
